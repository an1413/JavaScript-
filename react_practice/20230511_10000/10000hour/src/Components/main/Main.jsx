...
export default function Main(props) {
    ...
    const [trainingTime, setTraningTime] = useState(0);
    const [dday, setDday] = useState(0);


    const handleSubmit = (event) => {
        if(trainingTime < 0 || trainingTime > 24) {
            alert('다시 입력해주세요');
        }
        else {
  ...
  return (
      <main>
                ...
                <form className="cont-input" onSubmit={handleSubmit}>
                    ...
                    <input type="number" required value={trainingTime} onChange={(event) => setTraningTime(event.target.value)}/>
                    시간씩 훈련할 것이다.</p>
